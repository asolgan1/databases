...
LOAD DATA LOCAL INFILE './vaccination-small.txt'
...
LOAD DATA LOCAL INFILE './annualecondata-small.txt'
...
LOAD DATA LOCAL INFILE './coviddata-small.txt'
...
LOAD DATA LOCAL INFILE './annualcountrydata-small.txt'
...
WHERE netLB > low AND netLB < high AND year = 2021
ORDER BY netLB ASC;
END;$$          
								 
CREATE PROCEDURE showGDPGrowth(In country VARCHAR(15), IN allc VARCHAR(5))
BEGIN
