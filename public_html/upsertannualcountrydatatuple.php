<!-- 
    ...
    Upsert an annual country data tuple
-->
...
<body>
    ...
    $populationDensity = $_POST['populationDensity'] ? $_POST['populationDensity'] : NULL;

    if ($stmt = $conn->prepare("CALL UpsertAnnualCountryDataTuple(?, ?, ?, ?)")) {
        $stmt->bind_param("siid", $name, $year, $population, $populationDensity);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->field_count > 1) {
                ...
                echo "<p class=\"card-text\">Population: " . $row[3] . "</p>";
                echo "<p class=\"card-text\">Population Density: " . $row[4] . "</p>";
            }
