...
<body>
    ...
    $name = $_POST['name'];
    $month = $_POST['month'];
    $deaths = $_POST['deaths'] ? $_POST['deaths'] : NULL;
    $cases = $_POST['cases'] ? $_POST['deaths'] : NULL;


    if ($stmt = $conn->prepare("CALL UpsertCovidDataTuple(?, ?, ?, ?)")) {
        $stmt->bind_param("ssii", $name, $month, $deaths, $cases);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->field_count > 1) {
                ...
                echo "<p class=\"card-text\">Deaths: " . $row[3] . "</p>";
                echo "<p class=\"card-text\">Cases: " . $row[4] . "</p>";
            }
